...
public static class Utils{
	...
	private static readonly string REPO_URL = "https://github.com/titushm/swop";
	...
	public static bool EnsureProfileOverwrite(string profileName, ItemCollection items){
		...
		foreach (ListBoxItem item in items){
			if (item.Tag.ToString()?.ToLower() == profileName.ToLower()){
				MessageBoxResult result = MessageBox.Show("Are you sure you want to overwrite your profile?", "Overwrite Profile", MessageBoxButton.OKCancel);
				if (result != MessageBoxResult.OK) canceled = true;
				break;
			}
		}
		return canceled;
	...
	private static Task EnsurePath(string path){
